...
const UpdateForm: React.FC = () => {
  ...
  function handleUpdate(){
    if(data.id == -1)
    {
      ...
      console.log(newData);
    axios.post("http://localhost:9191/addProduct", newData,{
      headers: {
    ...
    {
      axios.put("http://localhost:9191/update", new Glasses(data.id,parseInt(count),color,shape,supplier,parseInt(quality),1),{
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
