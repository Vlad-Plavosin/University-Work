...
void FlagRCV_stateHandler(unsigned char byte) {
    ...
    switch (state.currentMode) {
        ...
        case RESPONSE_RR_REJ:
            if ((state.role == 0 && byte == MSG_A_RECV_RESPONSE) || (state.role == 1 && byte == MSG_A_TRANS_RESPONSE)) {
        ...
        case COMMAND_DISC:
        case COMMAND_DATA:
            if ((state.role == 1 && byte == MSG_A_TRANS_COMMAND) || (state.role == 0 && byte == MSG_A_RECV_COMMAND)) {
                state.currentState = A_RCV;
                state.address = byte;
                return;
            }
